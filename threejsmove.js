AFRAME.registerComponent("gaussian_splatting", {
				...
				try {
					...
					if (totalDownloadBytes != undefined) {
						...
						if (percent - lastReportedProgress > 1) {
							loadingText.textContent="Download Progress: "+ percent.toFixed(2) + "%", mbps.toFixed(2) + " Mbps";
					...
					const percentCheck=bytesDownloaded / totalDownloadBytes * 100;

					if(!isPly && totalDownloadBytes != undefined && bytesRemains > this.rowLength&&percentCheck>=99){
	...
	tick: function(time, timeDelta) {
		...
		const gauobj= document.getElementById('gauss');
		const orbitControls = camera.components['orbit-controls'];

		// console.log(gauobj.getAttribute('rotation'));
	},
